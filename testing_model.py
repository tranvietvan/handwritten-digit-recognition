...
for x in C:
    for y in gamma:
        #Make the model
        model_rbf=svm.SVC(kernel='rbf',C=x,gamma=y)

        #Traning + measuring time
        start = time.process_time()
        model_rbf.fit(train_test_X, train_test_Y)
        process_time = time.process_time() - start

        #Test with value
        prediction_val = model_rbf.predict(val_X)
        accurate_val = metrics.accuracy_score(val_Y, prediction_val)
        prediction_train = model_rbf.predict(train_X)
        accurate_train = metrics.accuracy_score(train_Y, prediction_train)

        #Print time need to train and accurate
        print("accuracy in train with C=", x, "gamma=", y, ": ", accurate_train)
        print("accuracy in validation with C=", x,"gamma=",y, ": ", accurate_val)
        print("time take to process: ", process_time)